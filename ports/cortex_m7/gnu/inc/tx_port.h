...
#define TX_THREAD_DELETE_EXTENSION(thread_ptr)                            \
  __iar_dlib_perthread_deallocate(thread_ptr->tx_thread_iar_tls_pointer); \
...
__attribute__((always_inline)) static inline void __set_control_value(ULONG control_value) { __asm__ volatile(" MSR  CONTROL,%0" : : "r"(control_value) : "memory"); }
...
__attribute__((always_inline)) void __set_control_value(ULONG control_value) { __asm__ volatile("MSR CONTROL,control_value"); }
...
#define TX_LOWEST_SET_BIT_CALCULATE(m, b)              \
  __asm__ volatile(" RBIT %0,%1 " : "=r"(m) : "r"(m)); \
...
#ifdef TX_PORT_USE_BASEPRI
__attribute__((always_inline)) static inline void __set_basepri_value(unsigned int basepri_value) { __asm__ volatile("MSR  BASEPRI,%0 " : : "r"(basepri_value)); }
#else
__attribute__((always_inline)) static inline void __enable_interrupts(void) { __asm__ volatile("CPSIE  i" : : : "memory"); }
#endif
...
#ifdef TX_PORT_USE_BASEPRI
static __inline void __set_basepri_value(unsigned int basepri_value) { __asm__ volatile("MSR  BASEPRI, #basepri_value"); }
#endif
...
static void _tx_thread_system_return_inline(void) {
  ...
  *((volatile ULONG *)0xE000ED04) = ((ULONG)0x10000000);
  if (_ipsr == 0) {
    ...
    __enable_irq();
    if (interrupt_save != 0) __disable_irq();
#endif
  }
}
...
CHAR _tx_version_id[] = "Copyright (c) 2024 Microsoft Corporation.  *  ThreadX Cortex-M7/GNU Version 6.4.1 *";
#else
#ifdef TX_MISRA_ENABLE
